...
for (let i = 0; i < blocks.length; i++) {
  ...
  const usablePage   = pdfHeight - topMargin - bottomMargin; 
  const spaceOnPage  = pdfHeight - bottomMargin - currentY;  
  const isShortBlock = imgHeight < usablePage * 0.65;        
  const isNearTop = currentY <= topMargin + 5; 
  
  if (isShortBlock && isNearTop) {
    const centeredY = topMargin + (usablePage - imgHeight) / 2;
    currentY = Math.max(topMargin, centeredY); 
  }
